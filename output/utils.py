...
def item_render_message(value, key=None):
    if key and re.match(MIME_PATTERN, key):
        ...
        resolved_outputs = json_dumps(value)
        resolved_outputs = resolved_outputs.replace("\\n", "\n").replace("\\t", "\t")
        resolved_outputs = resolved_outputs.replace("\\\"", "\"").replace("\\'", "'")
        resolved_outputs = resolved_outputs.replace("### ", "###")
        resolved_outputs = resolved_outputs.replace("#### ", "####")
        resolved_outputs = resolved_outputs.replace("## ", "##")
        resolved_outputs = resolved_outputs.replace("# ", "#")
        ...
        if key is None:
            st.markdown(f"{resolved_outputs},")
        else:
            st.markdown(f"{key}: {resolved_outputs},")
